...
def get_sports():
    ...
    return [s["sport"] for s in sports]


def validate_student(id_student, name_student):
    login_student = db.execute(
...
class Student:
    ...
    def find_available_sports(self, q):
        return db.execute("""
            SELECT * FROM sports WHERE sport LIKE ? AND id NOT IN (
                SELECT sport_id FROM registrations WHERE student_id = ?
            )
            """, q + "%", self.id)
    ...
    def unassign_sport(self, sport_id):
        db.execute(
            "DELETE FROM registrations WHERE student_id = ? AND sport_id = ?",
            self.id, sport_id,
        )
...
def home():
    ...
    return render_template(
        "home.html",
        name=name,
        enrolled_sports=student_enrolled_sports,
        unenrolled_sports=unenrolled_sports,
